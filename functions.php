...
// Procesar formulario via AJAX -- solo test
add_action('wp_ajax_enviar_presupuesto', 'procesar_formulario_presupuesto');
add_action('wp_ajax_nopriv_enviar_presupuesto', 'procesar_formulario_presupuesto');

function procesar_formulario_presupuesto() {
    // Verificar nonce de seguridad
    if (!wp_verify_nonce($_POST['nonce'], 'presupuesto_nonce')) {
    ...
    // Validaciones
    if (empty($nombre) || empty($email) || empty($material) || $metros <= 0) {
    ...
    // Crear el post del presupuesto (Custom Post Type opcional)
    $post_id = wp_insert_post([
        'post_title' => 'Presupuesto: ' . $nombre,
        'post_type' => 'presupuesto', // Necesitarías crear este CPT
        'post_status' => 'pending',
        'meta_input' => [
            'nombre' => $nombre,
            'email' => $email,
    ...
    $subject = 'Nueva solicitud de presupuesto - ' . get_bloginfo('name');
    $message = "
...
function tw_enqueue_scripts() {
    ...
    if ( is_front_page() ) {
        ...
        wp_enqueue_script(
            ...
            [],
            filemtime( get_template_directory() . '/assets/js/modules/hero-slider.js' ),
            true
        );
        ...
        wp_enqueue_script(
            'resenas-slider',
            get_template_directory_uri() . '/assets/js/modules/resenas-slider.js',
            [],
